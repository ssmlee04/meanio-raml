...
ExpressEngine.prototype.beginBootstrap = function(meanioinstance, database){
  ...
  .then(function (middleware) {
    ...
    // app.listen(3000)
    app.listen(config && config.http && config.http.port || 3000)
  }).catch(function(err){
    console.log(err);
  ...
  if (config.https && config.https.port && fs.existsSync(config.https.ssl.ca)) {
    var httpsOptions = {
    ...
    httpsServer.listen(config.https.port || 8000);
  }
