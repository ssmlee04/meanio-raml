...
Meanio.prototype.loadConfig = function(){
  var options = Meanio.prototype.options
  if (options && options.port && ~~options.port >= 3000) {
    this.config.clean.port = options.port
  }
  if (options && options.oldName && options.newName) {
    this.config.clean.root = this.config.clean.root.replace(options.oldName, options.newName)
  }
